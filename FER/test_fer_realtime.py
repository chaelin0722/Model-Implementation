...
def emotion(x, y):
    ...
    preds = x
    emotion_probability = y
    if preds.argmax() is None:
        print(" ")
    else:
    ...
    #cv2.putText(image, label, (int(left) + 10, int(top) - 10), cv2.FONT_HERSHEY_COMPLEX, 0.8,
...
# for image
'''
fpath = "../test_images/test_happy.jpg"
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        x_score, y_category = create_dataset(features_scores)
        n += 1

                ## normalization
                ## predict emotion
                #prediction = emotion(x_score, y_category)
        print("prediction", x_score)


    #  show original image + bbox
