## 24, 35 개가 아닌 모를 잡는 이미지는 바로 err 로 분류 때려버리는 코드
import os
import sys
import json
import datetime
...
DEFAULT_MRCNN_MODEL_DIR = "./mask_rcnn_toothbrush_head_0017.h5"  # val_loss: 0.5712 - val_rpn_class_loss: 0.0032 - val_rpn_bbox_loss: 0.1024 - val_mrcnn_class_loss: 0.1715 - val_mrcnn_bbox_loss: 0.0946 - val_mrcnn_mask_loss: 0.3060
DEFAULT_EFF_MODEL_DIR = 'NOAH/dataset/efficient-best_weight_220119_2.h5'
...
def binary_classification(imgname, model):
    ...
    if os.walk(test_dir).next()[2] == 25 or os.walk(test_dir).next()[2] == 34:
