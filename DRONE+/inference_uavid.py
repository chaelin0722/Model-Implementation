...
import random
import os
#python inference_uavid.py -i 'data/test' -c config/uavid/unetformer.py -o results/uavid -t 'lr' -ph 1152 -pw 1024 -b 2 -d uavid
'''
python inference_uavid.py -i data/test -c config/uavid/unetformer.py -o results/uavid -t lr -ph 1152 -pw 1024 -b 2 -d uavid
'''
...
def get_args():
    ...
    arg("-i", "--image_path", type=str, default='data/test/images', help="Path to  huge image")
...
def get_meta(img_dir):
    ...
    # GPS Latitude Of Image Center = Lat
    # GPS Longitude Of Image Center = Lon
    print(Lat, ",", Lon)

    return Lat, Lon, Altitude

def get_space(img_dir):
...
def main():
    ...
    for seq in seqs:
        ...
        for ext in ('*.tif', '*.png', '*.JPG'):
            img_paths.extend(glob.glob(os.path.join(args.image_path, str(seq), ext)))
        img_paths.sort()
        print("img_paths", img_paths)
        for img_path in img_paths:

            #### meta information ###
            #Lat, Lon, Altitude = get_meta(image_path)
            ####
        ...
        get_space("~/results/uavid/images/Labels/")
